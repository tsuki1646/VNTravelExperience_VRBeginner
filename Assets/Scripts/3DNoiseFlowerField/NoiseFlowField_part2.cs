...
public class NoiseFlowField_part2 : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < _amountOfParticles; i++)
        {
            ...
            while(attemp < 100)
            {
                ...
                Random.Range(this.transform.position.y, this.transform.position.y + _gridSize.x * _cellSize),
                Random.Range(this.transform.position.z, this.transform.position.z + _gridSize.x * _cellSize))
                ;
    ...
    void ParticleBehaviour()
    {
        foreach(flowfieldParticle p in _particles)
        {
            ...
            {
                p.transform.position = new Vector3(p.transform.position.x, p.transform.position.z, this.transform.position.z);
            }
