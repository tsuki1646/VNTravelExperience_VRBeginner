...
public class KochGenerator : MonoBehaviour
{
    ...
    [SerializeField]
    [Range(8,24)]
    protected int _bezierVertextCount;
    ...
    private void Awake()
    {
        ...
        _rotateVector = Quaternion.AngleAxis(_initialRotation, _rotateAxis)
                            * _rotateVector;

        for (int i = 0; i < _initiatorPointAmount; i++)
        {
            _position[i] = _rotateVector * _initiatorSize;
            _rotateVector = Quaternion.AngleAxis(360 / _initiatorPointAmount, _rotateAxis)
                            * _rotateVector;
        }
    ...
    protected void KochGenerate(Vector3[] positions, bool outwards, float generatorMultiplier)
    {
        ...
        _bezierPosition = BezierCurve(_targetPosition, _bezierVertextCount);
        _generationCount++;
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        for (int i = 0; i < _initiatorPointAmount; i++)
        {
            ...
            Matrix4x4 rotationMatrix = Matrix4x4.TRS(transform.position, transform.rotation,
